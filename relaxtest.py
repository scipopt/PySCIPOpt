...
def example():
    ...
    """
    #shows that relaxator is really used, time ~17 sec
    ...
    """
    """
    #SONC get some cutoff even though lowerbound < solution?? (leads to infeasibility even though SCIP without relaxator finds solution), time ~ 30 sec
    ...
    """
    """
    #(b) Test Polynomials modified, reasonable lower bound, time ~ 7sec
    m.addCons(-x0**4+3*x0-2*x1**2+1>=0)
    m.addCons(-x0**4-x1**4+42>=0)
    ...
    """
    """
    #Example 3.2 Ghasemi, Marshall, extended, time ~ 30sec
