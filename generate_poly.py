...
import aux
from aux import binomial, dt2sec
from polytope import convex_hull, is_in_convex_hull_cvxpy
...
def create_poly(n, degree, terms, A = None, hull_size = None, seed = None, negative = False, inner = 0):
	...
	if A is None:
		...
		for _ in range(max_count):
			...
			v = np.array(np.dot(A[:,:terms - inner], lamb / sum(lamb)).round(), dtype = np.int)
...
def create_simplex_polynomial(n,degree,terms,seed = None, negative = False):
	...
	for _ in range(max_attempts):
		A = 2*np.matrix(_create_exponent_matrix(n, d, n))
		A = np.matrix(make_affine(A))
		if np.linalg.matrix_rank(A[:,1:] - A[:,0]) < n:
			continue
		if np.linalg.det(A) < terms: continue

		inner = []
		for _ in range(terms**2):
			lamb = np.matrix(np.random.rand(n + 1)).T
			v = np.array((A * lamb / sum(lamb)).round(), dtype = np.int).T[0]
			new_lamb = np.linalg.solve(A,v)
