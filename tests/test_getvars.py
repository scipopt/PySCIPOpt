...
def test_getvars(idxvar):
    m, vs = setModel()
    if idxvar == []: # empty list
        assert not m.getVals([]), "no variable expected to be found"
    else:
        try:
            vs_ = [vs[i] for i in idxvar] # list of indices
        except TypeError:
            if idxvar is None:
                # just None
                vs_ = idxvar
                ...
                # an index
                vs_ = vs[idxvar]
        assert m.getVals(vs_), "list must be non-empty"
