...
def permutation_flow_shop(n,m,p):
    ...
    for j in range(1,n+1):
        model.addConstr(quicksum(x[j,k] for k in range(1,n+1)) == 1, "Assign1(%s)"%(j))
        model.addConstr(quicksum(x[k,j] for k in range(1,n+1)) == 1, "Assign2(%s)"%(j))
...
if __name__ == "__main__":
    ...
    seq = [j for (k,j) in sorted([(k,j) for (j,k) in x if x[j,k].X > 0.5])]
