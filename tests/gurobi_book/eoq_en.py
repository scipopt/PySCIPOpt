...
def eoq(I,F,h,d,w,W,a0,aK,K):
    ...
    for i in I:
        ...
        for k in range(K):
            w_[i,k] = model.addVar(ub=1, vtype="C", name="w(%s,%s)"%(i,k))
    model.update()

    for i in I:
        model.addConstr(quicksum(w_[i,k] for k in range(K)) == 1)
        model.addConstr(quicksum(a[i,k]*w_[i,k] for k in range(K)) == x[i])
        model.addConstr(quicksum(b[i,k]*w_[i,k] for k in range(K)) == c[i])
