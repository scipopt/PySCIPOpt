...
def mils_echelon(T,K,P,f,g,c,d,h,a,M,UB,phi):
    ...
    for t in Ts:
        for p in P:
            ...
            model.addConstr(E[t-1,p] + x[t,p] == E[t,p] + dsum, "FlowCons(%s,%s)"%(t,p))
    ...
    for p in P:
        ...
        for t in Ts:
            model.addConstr(E[t,p] >= quicksum(phi[p,q]*E[t,q] for (p2,q) in phi if p2 == p),
                            "EchelonLB(%s,%s)"%(t,p))
    ...
    if status == GRB.Status.UNBOUNDED:
        ...
        for p in P:
            ...
            for t in range(1,T+1):
                print "%7s%7s%7s%7s%12s%12s" % (p,t,d[t,p],int(y[t,p].X+0.5),round(x[t,p].X,5),round(I[t,p].X,5))
        print "\n"
        for k in K:
            for t in range(1,T+1):
                print "resource %3s used in period %s: %12s / %-9s" % (k,t,sum(a[t,k,p]*x[t,p].X+g[t,p]*y[t,p].X for p in P),M[t,k])
        ...
        for p in P:
            ...
            for t in range(1,T+1):
                print "%7s%7s%7s%7s%12s%12s" %\
                      (t,p,d[t,p],int(y[t,p].X+0.5),round(x[t,p].X,5),round(E[t,p].X,5))
        print "\n"
        for k in K:
            for t in range(1,T+1):
                print "resource %3s used in period %s: %12s / %-9s" % \
                      (k,t,sum(a[t,k,p]*x[t,p].X + g[t,p]*y[t,p].X for p in P),M[t,k])
