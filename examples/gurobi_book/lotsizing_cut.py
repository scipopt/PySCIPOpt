...
def sils(T,f,c,d,h):
    ...
    for t in Ts:
        model.addConstr(x[t] <= M*y[t], "ConstrUB(%s)"%t)
        model.addConstr(I[t-1] + x[t] == I[t] + d[t], "FlowCons(%s)"%t)
    ...
    while cuts:
        ...
        for ell in Ts:
            ...
            if lhs < D[1,ell]:
                ...
                model.addConstr(quicksum([x[t] for t in L]) +\
...
if __name__ == "__main__":
    ...
    y,x,I = model.__data
    model.optimize()
    print "\nOpt.value [standard]=",model.ObjVal
    print "%8s%8s%8s%8s%8s%8s%12s%12s" % ("t","fix","var","h","dem","y","x","I")
    for t in range(1,T+1):
        print "%8s%8s%8s%8s%8s%8s%12s%12s" % (t,f[t],c[t],h[t],d[t],y[t].X,x[t].X,I[t].X)

    model = sils_cut(T,f,c,d,h)
    y,x,I = model.__data
    print "\nOpt.value [cutting planes]=",model.ObjVal
    print "%8s%8s%8s%8s%8s%8s%12s%12s" % ("t","fix","var","h","dem","y","x","I")
    for t in range(1,T+1):
        print "%8s%8s%8s%8s%8s%8s%12s%12s" % (t,f[t],c[t],h[t],d[t],y[t].X,x[t].X,I[t].X)
