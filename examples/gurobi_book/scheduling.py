...
def scheduling_linear_ordering(J,p,d,w):
    ...
    for j in J:
        model.addConstr(quicksum(p[k]*x[k,j] for k in J if k != j) - T[j] <= d[j]-p[j], "Tardiness(%r)"%(j))
        ...
        for k in J:
            ...
            for ell in J:
                if ell > k:
                    model.addConstr(x[j,k] + x[k,ell] + x[ell,j] <= 2, "Triangle(%s,%s,%s)"%(j,k,ell))
...
def scheduling_time_index(J,p,r,w):
    ...
    for j in J:
        model.addConstr(quicksum(X[j,t] for t in range(1,T+1) if (j,t) in X) == 1, "JobExecution(%s)"%(j))

    for t in range(1,T+1):
        ind = [(j,t2) for j in J for t2 in range(t-p[j]+1,t+1) if (j,t2) in X]
        if ind != []:
            model.addConstr(quicksum(X[j,t2] for (j,t2) in ind) <= 1, "MachineUB(%s)"%t)

    model.setObjective(quicksum((w[j] * (t - 1 + p[j])) * X[j,t] for (j,t) in X), GRB.MINIMIZE)
...
def scheduling_disjunctive(J,p,r,w):
    ...
    for j in J:
        for k in J:
            if j != k:
                model.addConstr(s[j] - s[k] + M*x[j,k] <= (M-p[j]), "Bound(%s,%s)"%(j,k))
...
def scheduling_cutting_plane(J,p,r,w):
    ...
    model.addConstr(C[j]*p[j] >= sumP2/2 + (sumP**2)/2, "AllJobs")

    model.setObjective(quicksum(w[j]*C[j] for j in J), GRB.MINIMIZE)
    ...
    while True:
        ...
        cut += 1
        model.addConstr(quicksum(C[j]*p[j] for j in S) >= sumP2/2.0 + (sumP**2)/2.0, "Cut(%s)"%cut)
...
if __name__ == "__main__":
    ...
    for (i,j) in x:
        if x[i,j].X > .5:
            print "x(%s) = %s" % ((i,j), int(x[i,j].X+.5))
    for i in T:
        print "T(%s) = %s" % (i, int(T[i].X+.5))
    ...
    print "Opt.value by Time Index Formulation:",z
    seq = [j for (t,j) in sorted([(t,j) for (j,t) in X if X[j,t].X > .5])]
    ...
    seq = [j for (t,j) in sorted([(int(s[j].X+.5),j) for j in s])]
