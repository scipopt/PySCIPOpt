...
def eld_complete(U,p_min,p_max,d,brk):
    ...
    for u in U:
        ...
        model.addConstr(p[u] == quicksum(abrk[k]*z[k] for k in range(K+1)))
        model.addConstr(F[u] == quicksum(bbrk[k]*z[k] for k in range(K+1)))
        model.addConstr(quicksum(z[k] for k in range(K+1)) == 1)
        model.addSOS(GRB.SOS_TYPE2, [z[k] for k in range(K+1)])

    # demand satisfaction
    model.addConstr(quicksum(p[u] for u in U) == d, "demand")

    # objective
    model.setObjective(quicksum(F[u] for u in U), GRB.MINIMIZE)
...
if __name__ == "__main__":
    ...
    UB = sum(cost(a[u],b[u],c[u],e[u],f[u],p_min[u],p[u].X) for u in U)
