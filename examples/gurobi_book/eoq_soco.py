...
def eoq_soco(I,F,h,d,w,W):
    ...
    for i in I:
        model.addConstr(F[i] <= c[i]*T[i])

    model.addConstr(quicksum(w[i]*d[i]*T[i] for i in I) <= W)

    model.setObjective(quicksum(c[i] + h[i]*d[i]*T[i]/2. for i in I), GRB.MINIMIZE)
